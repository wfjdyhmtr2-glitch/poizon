...
/**
 * 其他费用类别候选（表单里下拉可选，也可直接手写新的）。
 * 得物侧常见：保证金、仓储费、取回费、鉴定费；各平台共通：会员费、推广费、软件服务费。
 * 历史记录里用过的类别会自动并进候选，所以这份清单只是「打底」，不必穷举。
 */
...
/** 常见费用平台（卖货平台 + 进货平台都可能产生费用；历史用过的会自动并进来） */
export const OTHER_EXPENSE_PLATFORMS = [
  "得物",
  "淘宝",
  "天猫",
  "1688",
...
/**
 * 把「内置候选」和「历史记录里真实出现过的值」合并去重。
 * 这样任何一个新填过的类别 / 平台，下次都会自动出现在下拉里。
 */
export function mergeOptions(builtin: string[], used: (string | null | undefined)[]): string[] {
  const seen = new Set<string>()
  const out: string[] = []
  for (const raw of [...builtin, ...used]) {
    const v = (raw ?? "").trim()
